...
namespace WebShop
{
    internal class Window
    {
        ...
        public void Draw()
        {
            var longestRow = TextRows.OrderByDescending(s => s.Length).FirstOrDefault(); // to be deleted
            //var width = TextRows.OrderByDescending(s => s.Length).FirstOrDefault().Length; //undo 
            var width = longestRow != null ? longestRow.Length : 0; // to be deleted
            // Kolla om Header är längre än det längsta ordet i listan
            if (Header != null && width < Header.Length + 4) //Header != null && to be deleted
            {
